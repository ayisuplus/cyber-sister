...
export function requireLocalWorkRuntime() {
  if (!isLocalWorkRuntime()) throw Object.assign(new HttpError('网页版仅支持聊天，工作模式需使用本地客户端', 403), { code: 'LOCAL_CLIENT_REQUIRED' })
}
export function localWorkOnly(_req, res, next) {
  if (!isLocalWorkRuntime()) return res.status(403).json({ error: '网页版仅支持聊天，工作模式需使用本地客户端', code: 'LOCAL_CLIENT_REQUIRED' })
  return next()
}
