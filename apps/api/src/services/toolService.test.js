...
describe('待办', () => {
  ...
  it('只更新显式传入的字段', async () => {
    ...
    expect(db.todoUpdate).toHaveBeenLastCalledWith({
      where: { id: 't1' },
      data: { content: '改名', dueDate: null, dueTime: null },
    })
  })
...
describe('经期记录', () => {
  ...
  it('拒绝不存在的日期、倒置范围和非布尔日程状态', async () => {
    ...
    await expect(createCountdown('u1', { title: ' ', targetDate: '2026-09-12' })).rejects.toMatchObject({ statusCode: 400 })
    await expect(createTodo('u1', { content: '测试', dueDate: '2026-02-30' })).rejects.toMatchObject({ statusCode: 400 })
    db.todoFindFirst.mockResolvedValue({ id: 't1' })
    await expect(updateTodo('u1', 't1', { isDone: 'false' })).rejects.toMatchObject({ statusCode: 400 })
