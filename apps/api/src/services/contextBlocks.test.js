...
describe('只是倾诉的一轮', () => {
  ...
  it('倾诉里常见的说法也算，不只认难过、生气、焦虑那几个词', () => {
    // 句子特意不用回复质量评测集里的原话，免得只是对着考题改
    for (const text of [
      '一个人在家待了一整天，好孤单',
      '昨天跟我妈吵完架，现在很愧疚',
      '开会的时候出了个大丑，太丢脸了',
      ...
      '他不让我跟朋友出去玩',
      '体检报告出来了，吓死我了',
    ]) expect(isFeelingTurn(text)).toBe(true)
  })

  it('带着情绪但明说交代办事，照常给工具', () => {
    for (const text of ['好孤单，帮我定个周末早上的闹钟', '好紧张，明天穿什么', '很愧疚，帮我写一条道歉的消息']) {
