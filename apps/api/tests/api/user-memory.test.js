...
describe('用户、同意与显式记忆 API', () => {
  ...
  it('同意状态初始为未选择，并可记录拒绝与接受', async () => {
    ...
    expect(initial.body).toEqual({ accepted: null, version: 'qwen-fallback-v1', updatedAt: null })
    expect(declined.body).toMatchObject({ accepted: false, version: 'qwen-fallback-v1' })
    expect(accepted.body).toMatchObject({ accepted: true, version: 'qwen-fallback-v1' })
  ...
  it('旧版本同意会自动回到未选择状态', async () => {
    ...
    expect(res.body).toEqual({ accepted: null, version: 'qwen-fallback-v1', updatedAt: null })
  })
