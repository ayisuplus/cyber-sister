...
export default function BeautyCameraPage() {
  ...
  const handleSave = () => {
    ...
    canvas.toBlob((blob) => {
      ...
      link.href = url
      link.download = `美颜相机-${format(new Date(), 'yyyy-MM-dd-HHmm')}.png`
      link.click()
      ...
      <main className="flex-1 overflow-y-auto px-4 py-5">
        <section className="rounded-3xl bg-pastel-mist p-5 shadow-card">
          <div className="flex items-start gap-3">
            ...
            <div>
              <h1 className="text-base font-semibold text-text-primary">美颜相机</h1>
              <p className="mt-1 text-xs leading-relaxed text-text-secondary">
                拍一张或选一张照片，实时调整磨皮、美白、瘦脸、大眼。
              </p>
