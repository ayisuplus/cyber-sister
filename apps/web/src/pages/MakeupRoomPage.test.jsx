...
const { processImageMock, getUserMediaMock, makeupPresetApi } = vi.hoisted(() => ({
  processImageMock: vi.fn(),
  getUserMediaMock: vi.fn(),
  makeupPresetApi: { list: vi.fn(), create: vi.fn(), rename: vi.fn(), remove: vi.fn() },
}))
...
vi.mock('../services/makeupPresetService', () => ({ makeupPresetService: makeupPresetApi }))
...
function renderPage() {
  return render(<MemoryRouter><MakeupRoomPage /></MemoryRouter>)
}
...
beforeEach(() => {
  makeupPresetApi.list.mockReset()
  makeupPresetApi.list.mockResolvedValue([])
  makeupPresetApi.create.mockReset()
  makeupPresetApi.rename.mockReset()
  makeupPresetApi.remove.mockReset()
...
describe('自定义妆容预设', () => {
  const CUSTOM = { id: 'c1', name: '日常', smooth: 30, whiten: 20, slim: 10, eye: 10 }

  it('加载后自定义预设出现在预设横排', async () => {
    makeupPresetApi.list.mockResolvedValue([CUSTOM])

    await captureStill()

    const group = screen.getByRole('group', { name: '美颜预设' })
    expect(await within(group).findByRole('button', { name: '日常' })).toBeInTheDocument()
  })

  it('点自定义预设应用其四项值', async () => {
    makeupPresetApi.list.mockResolvedValue([CUSTOM])
    await captureStill()
    const group = screen.getByRole('group', { name: '美颜预设' })
    const presetButton = await within(group).findByRole('button', { name: '日常' })
    processImageMock.mockClear()
    ...
    expect(processImageMock).toHaveBeenLastCalledWith(
      expect.anything(),
      { smooth: 30, whiten: 20, slim: 10, eye: 10 },
      LANDMARK_KEY_ARG,
    )
    expect(presetButton).toHaveAttribute('aria-pressed', 'true')
  })

  it('把当前存为妆容后出现在预设横排', async () => {
    makeupPresetApi.create.mockImplementation(async (payload) => ({ id: 'c2', ...payload }))
    await captureStill()

    fireEvent.click(screen.getByRole('button', { name: '把当前存为妆容' }))
    fireEvent.change(screen.getByLabelText('妆容名字'), { target: { value: '通勤' } })
    fireEvent.click(screen.getByRole('button', { name: '保存' }))

    await waitFor(() => expect(makeupPresetApi.create).toHaveBeenCalledWith({ name: '通勤', ...NATURAL_SETTINGS }))
    const group = screen.getByRole('group', { name: '美颜预设' })
    expect(await within(group).findByRole('button', { name: '通勤' })).toBeInTheDocument()
    expect(screen.queryByRole('alertdialog')).not.toBeInTheDocument()
  })

  it('删除经确认后从列表消失', async () => {
    makeupPresetApi.list.mockResolvedValue([CUSTOM])
    makeupPresetApi.remove.mockResolvedValue({ success: true })
    await captureStill()
    const group = screen.getByRole('group', { name: '美颜预设' })
    expect(await within(group).findByRole('button', { name: '日常' })).toBeInTheDocument()
    ...
    await waitFor(() => expect(makeupPresetApi.remove).toHaveBeenCalledWith('c1'))
    await waitFor(() => expect(within(group).queryByRole('button', { name: '日常' })).not.toBeInTheDocument())
  })
})
describe('性能自适应降级', () => {
