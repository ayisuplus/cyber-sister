  ...
  return (
    <div className="flex items-center gap-3 px-4 py-3 border-b border-border-subtle last:border-0">
      ...
      <div className="flex-1 min-w-0">
        <p title={todo.content} className={`text-sm truncate ${todo.isDone ? 'text-text-muted line-through' : 'text-text-primary'}`}>{todo.content}</p>
...
// 日程面板：原 TodoPage 主体，挂在 PlannerPage 的「日程」页签下
        ...
        {showInput ? (
          <div className="space-y-2">
            <input
              ...
              aria-label="日程内容"
              autoFocus
              className="w-full min-h-11 bg-surface-input rounded-xl px-4 text-sm outline-none focus-visible:ring-2 focus-visible:ring-status-info"
            />
            <div className="flex gap-2">
              <input
                ...
                aria-label="日期（可选）"
                className="flex-1 min-h-11 bg-surface-input rounded-xl px-3 text-sm text-text-primary outline-none focus-visible:ring-2 focus-visible:ring-status-info"
              />
              <input
                type="time"
                value={newTime}
