...
export default function Header({ title, showBack = false, rightAction = null }) {
  ...
  return (
    ...
    <div className="flex items-center justify-between px-4 h-14 bg-surface-card border-b border-border-hairline shrink-0">
      <div className="w-10">
        {showBack && (
          <button type="button" aria-label="返回上一页" onClick={() => navigate(-1)} className="flex h-11 w-11 items-center justify-center rounded-xl transition-colors hover:bg-surface-muted focus-visible:ring-2 focus-visible:ring-status-info active:scale-95">
