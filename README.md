...
- AI 角色持续状态与增量学习：本地评价、注意力预算、成功回合后保存成长；设置页“她的状态”可查看和恢复节奏。数值模拟的范围、所需迁移与验证限制见 [角色运行内核](docs/architecture/pi-companion-runtime-20260915.md)。
- `cloud-primary-v3` 云端模型同意、拒绝与撤回：未同意时云端调用次数为零，聊天不可用；每次调用前服务端重读同意状态，撤回授权会拦截尚未发出的请求。v3 覆盖聊天内容、照片与记忆内容用于生成语义向量，旧版同意全部失效需重新同意。
...
- 智能体工具回路：聊天内直接操作上述工具（日程/倒数日/经期/提醒/日记/手帐/阅读/自习/联网搜索），动作标签可见；工作模式为日程四件、计算换算、联网搜索、写作/翻译/计划。工作模式未选会话时主区直出「功能桌面」网格（全部功能入口：日记/日程/经期/倒数日/手帐/阅读/自习/工作台/提醒设置/3D 衣柜/化妆间），选中会话后 ChatHeader「功能」按钮随时唤出同一面板。
- 化妆间：MediaPipe WASM 全程浏览器内处理，照片不上传；四滑杆组合可存为命名妆容预设，服务端落库多设备同步（`/api/makeup-presets`）。
- 3D 衣柜：上传单品照片生成可旋转预览的 GLB 模型（`<model-viewer>`）。图生 3D 走外部服务（`IMAGE_TO_3D_API_URL` / `IMAGE_TO_3D_API_KEY`），未配置时如实 503，不落任何数据；单品图与模型存 `apps/api/data/wardrobe/`（`WARDROBE_DIR` 可覆盖）。
...
- 她来想你（主动关怀）：打开 App 时基于真实数据的关怀卡（生日/经期/倒数日/日程/打卡/自习/昨日心情），附「为什么看到这条」、按日可忽略、设置页真实总开关；in-app 拉取，无推送通道。
- 她的信：每周一封，完全由本周真实数据本地生成（不调用云端模型），同一周幂等唯一，沉默周宁缺毋滥。
- 数据与迁移：导出全部自有数据为单个 JSON（永久免费，不设会员门槛）；导入自家导出包或豆包/千问智能体人设文本，逐条确认后落库；角色扮演恋人红线如实拒绝。
- 中高风险输入服务端阻断；危机资源文案须经产品负责人核验后配置。

天气（现有路由为假数据，内测环境保持 409 关闭）、会员、支付、账号删除、真实短信、自动记忆提取（语义投影已实现，自动落库式提取仍后置）、模型训练、微服务/Kubernetes 和公开发布均不属于本次内测。
