...
> 🚫 **状态：历史归档（2026-09-16 已被取代）**。产品负责人决定前后端都落在同一台服务器：改用 `compose.yaml` + `deploy/compose.server.yaml`（edge nginx 同源发页面并反代 `/api`），部署由 CI/CD 流水线执行，见 [`docs/04-开发/CI-CD流水线.md`](../../04-开发/CI-CD流水线.md)。
