...
状态：⚠️ 参考 · 2026-09-15 阶段记录（2026-09-17 标注）。保留角色内核设计与当时验证边界；当前状态入口已移至「她」，旧双模式描述以 [Spec](../Spec_Amie_v1.0.md) 为准。本文的个人环境迁移状态未在本轮复核。
...
> 2026-09-21 更新（路线图第二步）：面板移到「她」页，只写真的在变的——不平常的节奏（此时才出现“恢复平稳节奏”）与学到的长短偏向，不再显示计数，信任不显示。内核接上真实输入（模糊的长短要求、道谢、冲着她的重话、隔半小时以上回来），冒犯按连续句数进入放慢/简短，状态新增 `session.startedAt`；`companionPacing(state, moment)` 按本轮临时输入（北京时间、间隔、这次聊了多久、心情、单独同意后的经期）给出【这一轮的分寸】，临时输入不写进状态。
