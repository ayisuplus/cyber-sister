...
- **历史归档陷阱**：`docs/` 全目录中带「历史归档 / 未来方案」类横幅的文档——典型是已物理移入 `docs/09-参考/历史归档/` 的 8 份（API 网关、后端架构重构、安全防护、性能优化、数据库架构、架构设计总结、实施路线图、实现总结）——描述的是微服务 / K8s / Redis / 向量库等**未在仓库实现**的架构，实施时一律以 `Spec_Amie_v1.0.md` 与源码为准。
